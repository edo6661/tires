...
<div class="bg-gradient-to-r from-blue-50 to-indigo-50 border-b border-gray-200">
    <div class="px-6 pt-4">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-xl font-semibold text-gray-800">{{ __('admin/reservation/calendar.title') }}</h2>
            <div class="flex items-center space-x-2">
                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                    ...
                    {{ __('admin/reservation/calendar.status.active') }}
                </span>
            </div>
        </div>
        ...
        <div class="flex justify-between gap-4 flex-wrap">
            <div class="flex space-x-1" role="tablist" aria-label="Tabs">
                ...
                aria-selected="{{ request('tab', 'calendar') === 'calendar' ? 'true' : 'false' }}">
                    ...
                    {{ __('admin/reservation/calendar.tabs.calendar_view') }}
                ...
                aria-selected="{{ request('tab') === 'list' ? 'true' : 'false' }}">
                    ...
                    {{ __('admin/reservation/calendar.tabs.list_view') }}
                    @if(request('tab') === 'list')
                        <span class="absolute bottom-0 left-0 right-0 h-0.5 bg-blue-500 rounded-t-full"></span>
                    @endif
                </a>
            ...
            <div>
                <a href="{{ route('admin.reservation.create') }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm transition-colors duration-200">
                    ...
                    {{ __('admin/reservation/calendar.buttons.new_reservation') }}
                </a>
            </div>
        </div>
    </div>
        ...
        <div class="flex items-center space-x-2 text-sm text-gray-600">
            @if(request('tab', 'calendar') === 'calendar')
                <span class="inline-flex items-center">
                    ...
                    {{ __('admin/reservation/calendar.mode_description.calendar') }}
                </span>
            @else
                <span class="inline-flex items-center">
                    <span class="w-2 h-2 bg-blue-400 rounded-full mr-2"></span>
                    {{ __('admin/reservation/calendar.mode_description.list') }}
                </span>
            @endif
        </div>
        
        <div class="flex items-center space-x-2">
            <button class="text-gray-400 hover:text-gray-600 transition-colors" title="{{ __('admin/reservation/calendar.buttons.refresh') }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/>
                </svg>
            </button>
            <button class="text-gray-400 hover:text-gray-600 transition-colors" title="{{ __('admin/reservation/calendar.buttons.settings') }}">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
