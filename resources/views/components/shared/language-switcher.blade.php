...
@php
    ...
    $dropdownPosition = isset($positionClasses[$position]) ? $positionClasses[$position] : $positionClasses['bottom-right'];
@endphp
@if($style === 'dropdown')
        ...
        >
            @if($showFlag && $currentLocaleInfo)
                @if(isset($flagImages[$currentLocale]))
                    <img src="{{ $flagImages[$currentLocale] }}" alt="{{ $currentLocaleInfo['name'] }} flag" class="w-4 h-3 object-cover rounded-sm">
                @else
            ...
            <div class="py-1">
                @foreach($supportedLocales as $localeCode => $localeInfo)
                    @php
                        ...
                        $routeParams = array_merge(request()->route()->parameters(), ['locale' => $localeCode]);
                    @endphp
                    ...
                    >
                        @if($showFlag)
                            @if(isset($flagImages[$localeCode]))
                                <img src="{{ $flagImages[$localeCode] }}" alt="{{ $localeInfo['name'] }} flag" class="w-5 h-4 object-cover rounded-sm flex-shrink-0">
                            @else
                        ...
                        <div class="flex flex-col">
                            ...
                            <span class="text-xs text-gray-500 uppercase">{{ $localeCode }}</span>
                        </div>
                        @if($isActive)
    ...
    <div class="flex items-center {{ $compact ? 'gap-1' : 'gap-2' }}">
        @foreach($supportedLocales as $localeCode => $localeInfo)
            ...
            >
                @if($showFlag)
                    @if(isset($flagImages[$localeCode]))
                        <img src="{{ $flagImages[$localeCode] }}" alt="{{ $localeInfo['name'] }} flag" class="{{ $compact ? 'w-4 h-3' : 'w-5 h-4' }} object-cover rounded-sm">
                    @else
        ...
        >
            @foreach($supportedLocales as $localeCode => $localeInfo)
                @php
                    ...
                    $routeParams = array_merge(request()->route()->parameters(), ['locale' => $localeCode]);
                @endphp
                <option value="{{ route($routeName, $routeParams) }}"
                        {{ $isActive ? 'selected' : '' }}>
                    @if($showFlag && !isset($flagImages[$localeCode])){{ $localeInfo['flag'] }} @endif
                    {{ $localeInfo['name'] }}
                    @if(!$showName)({{ strtoupper($localeCode) }})@endif
                </option>
            @endforeach
        </select>
